...
object MRZCleaner {
    ...
    fun clean(mrz: String): String {
        ...
        if (result.contains("<") && (
            ...
            Log.d(MRZActivity.TAG, "Error = [${URLEncoder.encode(result, "UTF-8").replace("%3C", "<").replace("%0A", "↩")}]")
    ...
    fun parseAndClean(mrz: String): MrzRecord {
        var record = MrzParser.parse(mrz)

        Log.d(MRZActivity.TAG, "Previous Scan: $previousMRZString")
        if (!record.validComposite || !record.validDateOfBirth || !record.validDocumentNumber || !record.validExpirationDate) {
            ...
            Log.d(MRZActivity.TAG, "Still accept scanning.")
        }
